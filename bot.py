...
def updateCallback(client, callback_query,redis):
	...
	if date == "bkk":
		...
		app.edit_message_text(chatID, message_id,"• اهلا بك في بوت الرفع .\n• يمكنك ارسال رسائل بريدية عن طريق اضافه اميلات هذا البوت تابع لـ خالد", disable_web_page_preview=True, parse_mode=ParseMode.MARKDOWN,reply_markup=kup)
	if date == "spam":
...
def updateHandlers(client, message,redis):
	...
	if userID == SUDO:
		if text ==  '/start':
			kup = InlineKeyboardMarkup([[InlineKeyboardButton("تعيين اميل",callback_data="setmail"),InlineKeyboardButton("تعيين عنوان",callback_data="setTit"),InlineKeyboardButton("تعيين كليشة",callback_data="settxt")],[InlineKeyboardButton("بدء spam",callback_data="spam")],[InlineKeyboardButton("العمالقة",url="T.ME/TM_BOYKA")]])
			message.reply_text("• اهلا بك في بوت الرفع .\n• يمكنك ارسال رسائل بريدية عن طريق اضافه اميلات هذا البوت تابع لـ خالد", quote=True, disable_web_page_preview=True, parse_mode=ParseMode.MARKDOWN,reply_markup=kup)
